...
class PinholeDistortK1 final : public Camera
{
public:
	enum {
		OFFSET_FOCAL_X,
	...
	struct Cost
	{
		...
		bool operator()(const T* const intrinsic, const T* const extrinsic, const T* const pos3d, T* residuals) const
		{
			...
			const T radial_coeff = T(1.0) + k1 * r2;
			x *= radial_coeff;
			y *= radial_coeff;
	...
	PinholeDistortK1(Eigen::Matrix<double, 5, 1>& intrinsic)
	{
		//Copy intrinsic
	...
	{
		return new ceres::AutoDiffCostFunction<PinholeDistortK1::Cost, 2, 5, 6, 3>(
			new PinholeDistortK1::Cost(observation)
			);
	}
